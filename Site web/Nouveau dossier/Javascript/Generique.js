...
// Nom : ajouterTextarea_ToUl_Selectable
// Par : Mathieu Dumoulin, modifié par Isabelle Angrignon
// Date : 01/12/2014
// Intrant(s) : String idUl, String element,String idElement , bool estThemeJqueryUI
// Extrant(s) : Il n'y a pas d'extrant
// Description : Cette fonction prend l'id d'une balise Ul et lui ajoute un textarea créé dynamiquement comportant le texte (element) passer en paramêtre
//				 en donnant les classe pour le thème d'un selectable au textarea si estThemeJqueryUI est à true.//
function ajouterTextarea_ToUl_Selectable(idUl, element,idElement, estThemeJqueryUI, classTextArea) {
    // Initialisation du textarea
    var textareaTag = document.createElement("textarea");
    textareaTag.setAttribute("id",idElement);
    if(estThemeJqueryUI) {
        textareaTag.setAttribute("class", "ui-state-default selectable", classTextArea);
    }
    else{
        textareaTag.setAttribute(classTextArea);
    }
    $(textareaTag).val(element).attr("readonly", true).attr("rows", 1);
    document.getElementById(idUl).appendChild(textareaTag);
}
